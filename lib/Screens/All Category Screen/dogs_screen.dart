...
import 'package:le_vech/Controller/All%20Screen%20Controller/dogs_controller.dart';
...
class _DogsScreenState extends State<DogsScreen> {
  DogsController dogsController = Get.put(DogsController());

  @override
  void initState() {
    setState(() {});
    dogsController.sellAllDogs(context);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
        ...
        child: Obx(() {
          return dogsController.isLodingDogs.value
              ? const CircularProgressIndicator()
              : dogsController.sellDogs.isNotEmpty
                  ? GridView.builder(
                      itemCount: dogsController.sellDogs.length,
                      shrinkWrap: true,
...
class _ItemWidgetState extends State<ItemWidget> {
  DogsController dogsController = Get.put(DogsController());
  List favDogsTempList = [];

  @override
  void initState() {
    favDogsTempList = dogsController.favDogsList[widget.index];
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      return InkWell(
          onTap: () {
            Navigator.of(context).push(MaterialPageRoute(builder: (context) => LeVechProfile(detail: dogsController.sellDogs[widget.index])));
          },
          child: Card(
              elevation: 2,
                  ...
                  child: Column(children: [
                    ...
                    Card(
                        ...
                        child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              image: DecorationImage(image: NetworkImage(dogsController.sellDogs[widget.index]["item_img"][0].toString().isNotEmpty
                                  ? dogsController.sellDogs[widget.index]["item_img"][0].toString()
                    ...
                    Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          AppText(text: dogsController.sellDogs[widget.index]["name"],txtColor: AppColor.primarycolorblack,size: 16,fontWeight: FontWeight.w600,overflow: TextOverflow.ellipsis),
                          ...
                          Row(children: [
                            Expanded(
                                child: Text(dogsController.sellDogs[widget.index]["price"],
                            ...
                            InkWell(
                                ...
                                child: Icon(favDogsTempList.contains(userId) ? Icons.favorite : Icons.favorite_border,
                                    color: favDogsTempList.contains(userId) ? AppColor.iconColor : AppColor.primarycolorblack, size: 24))
                          ])
                        ])),
                    const SizedBox(height: 5),
                    AppText(text: dogsController.sellDogs[widget.index]["item_type"],txtColor: AppColor.grey700,size: 13),
                  ]))));
    });
  }
}
